...
> **Source:** [Anthropic API Documentation](https://docs.anthropic.com/) & [LLM Evaluation Best Practices](https://www.promptingguide.ai/)

### Phase 1: Discovery
- [ ] **Audit**: List all model strings (`claude-3-opus`, `claude-3-5-sonnet`) in the codebase.
- [ ] **Test Cases**: Run the "Golden Prompt" set to measure model success before migration (Baseline).

### Phase 2: Update & Refinement
- [ ] **String Replacement**: Replace Model IDs with `claude-3-5-opus-20241022` (or current ID).
- [ ] **Parameter Check**: Check Temperature and Max Tokens values (new model might be more verbose or creative).
- [ ] **System Prompt**: The new model's sensitivity in understanding instructions might have changed; simplify the system.

### Phase 3: Validation
- [ ] **Regression Test**: Re-run Golden Prompt set and compare outputs.
- [ ] **Cost Analysis**: Analyze change in token usage (producing less/more tokens?).
- [ ] **Latency Check**: Measure change in response time.

### Checkpoints
| Phase | Verification                                          |
| ----- | ----------------------------------------------------- |
| 1     | Did the new model's "refusal" rate increase?          |
| 2     | Are JSON outputs still valid?                         |
| 3     | Does cost increase (if any) justify performance gain? |
