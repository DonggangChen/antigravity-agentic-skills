...
> **Source:** [Platform Engineering Community](https://platformengineering.org/) & [Team Topologies](https://teamtopologies.com/)

### Phase 1: Discovery & Strategy (Product Mindset)
- [ ] **User Research**: Identify developers' biggest pain points (e.g., Env provisioning is too slow).
- [ ] **TVP (Thinnest Viable Platform)**: Do not automate everything. Choose a "Golden Path" first (e.g., Spring Boot on K8s).
- [ ] **Capabilities**: Portal (Backstage/Port) or just CLI? Decide according to need.

### Phase 2: Implementation (Golden Paths)
- [ ] **Template**: Create project template with `cookiecutter` or Backstage Scaffolder.
- [ ] **Orchestration**: Separate App Config from Infrastructure (Terraform/Crossplane) (Score/Workload spec).
- [ ] **Portal**: Connect service catalog and documentation to a central portal.

### Phase 3: Adoption & Evolution
- [ ] **Marketing**: Market the platform like a product (Internal Demos, Newsletters).
- [ ] **Metrics**: Measure DORA and SPACE metrics. Track platform adoption rate.
- [ ] **Feedback Loop**: Get regular feedback from developers and update roadmap.

### Checkpoints
| Phase | Verification                                                                 |
| ----- | ---------------------------------------------------------------------------- |
| 1     | Is platform team a "blocker" or "accelerator" for developers?                |
| 2     | Is "Cognitive Load" reduced? (Does developer have to write Kubernetes YAML?) |
| 3     | Is documentation sufficient to be "Self-service"?                            |
