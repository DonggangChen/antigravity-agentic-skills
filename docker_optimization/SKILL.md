...
> **Source:** [Docker Build Best Practices](https://docs.docker.com/build/building/best-practices/) & [Trivy Docs](https://aquasecurity.github.io/trivy/)

### Phase 1: Base & Structure
- [ ] **Base Image**: Select an `-alpine` or `-slim` image for production (Pin version: `python:3.11.9-slim`).
- [ ] **Layers**: Move unchanging layers (Dependency Install) up, move code copy (`COPY . .`) to the bottom.
- [ ] **Multi-Stage**: Leave build tools (`gcc`, `npm`) in the builder stage, do not move them to the runtime stage.

### Phase 2: Security & Linting
- [ ] **Linter**: Scan Dockerfile with `hadolint` (`hadolint Dockerfile`).
- [ ] **User**: Switch to a non-root user with `USER appuser`.
- [ ] **Secrets**: Do not pass secrets with `ENV`, use secret mounts.

### Phase 3: Performance Check
- [ ] **Context**: Does `.dockerignore` exclude `.git`, `node_modules`, and tests?
- [ ] **Cache**: Speed up package manager cache using `RUN --mount=type=cache`.
- [ ] **Scan**: Scan the image with `trivy image <name>` and fix critical vulnerabilities.

### Checkpoints
| Phase | Verification                                                           |
| ----- | ---------------------------------------------------------------------- |
| 1     | Is the image size 50%+ smaller than the builder stage?                 |
| 2     | Are there any hidden files/keys left when checked with `dive <image>`? |
| 3     | Can the container run without root?                                    |
