...
> **Source:** [PCI DSS v4.0 Quick Reference Guide](https://www.pcisecuritystandards.org/) & [Stripe Integration Guide](https://stripe.com/docs/development)

### Phase 1: Compliance & Architecture
- [ ] **Scope**: Determine SAQ (Self-Assessment Questionnaire) type (A, A-EP, D).
- [ ] **Tokenization**: "Raw card data" should never touch your server. Use Stripe Elements or iframe.
- [ ] **Idempotency**: Add unique `Idempotency-Key` header to every payment request (Prevent double charge).

### Phase 2: Implementation Flows
- [ ] **Intent**: Use Payment Intent pattern (Asynchronous confirmation for SCA/3D Secure compliance).
- [ ] **Webhooks**: Confirm payment result (success/fail) *only* with data from webhook (Client-side is unreliable).
- [ ] **Signature Verification**: Verify incoming webhook comes from provider using secret.

### Phase 3: Hardening
- [ ] **Logging**: Clean/mask PII (Personal data) and card data from logs.
- [ ] **Testing**: Simulate all scenarios (Success, Decline, 3DS, Insufficient Funds) with test cards.
- [ ] **Reconciliation**: Set up a job producing daily reconciliation report.

### Checkpoints
| Phase | Verification                                                                        |
| ----- | ----------------------------------------------------------------------------------- |
| 1     | Is TLS 1.2+ mandatory against "Man-in-the-middle" attacks?                          |
| 2     | Does system retry if webhook endpoint does not return 200 OK?                       |
| 3     | What is the transaction status if customer drops off (internet cut) during payment? |