...
> **Source:** [Zustand v5.0 Official Documentation](https://zustand-demo.pmnd.rs/) & [Next.js Hydration Patterns](https://zustand.docs.pmnd.rs/guides/nextjs)

### Phase 1: Store Definition & Types
- [ ] **Model Selection**: Determine TypeScript interfaces containing State and Action structures.
- [ ] **Curry Initialization**: Initialize store using `create<T>()()` (double parentheses) syntax.
- [ ] **Middleware Selection**: Add `persist` (localStorage) or `devtools` layers depending on need.

### Phase 2: React Integration & Slices
- [ ] **Atomic Slices**: Split large stores into atomic slices using `StateCreator`.
- [ ] **Selector Strategy**: Ensure components subscribe only to the state slice they use (`useStore(state => state.X)`).
- [ ] **Shallow Audit**: Use `shallow` when selecting multiple values to prevent unnecessary renders.

### Phase 3: Persistence & Hydration
- [ ] **Hydration Guard**: Ensure SSR compatibility with `_hasHydrated` flag pattern in Next.js projects.
- [ ] **Storage Config**: Configure `sessionStorage` or custom encrypted storage for sensitive data.
- [ ] **Action Logging**: Monitor action names (`counter/increase`) with devtools during debugging process.

### Checkpoints
| Phase | Verification                                                                    |
| ----- | ------------------------------------------------------------------------------- |
| 1     | Was direct mutation performed inside `set` method? (Illegal!)                   |
| 2     | Did component use direct destructuring instead of selector? (Performance risk!) |
| 3     | Is Persist storage key unique?                                                  |
