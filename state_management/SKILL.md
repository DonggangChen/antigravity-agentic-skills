...
> **Source:** [Redux Documentation - State Management Best Practices](https://redux.js.org/style-guide/) & [React State Management Guide](https://react.dev/learn/managing-state)

### Phase 1: State Scoping & Selection
- [ ] **Identify State Type**: Determine state type (Server State, UI State, Form State, Global State).
- [ ] **Tool Selection**: Select correct tool among `useState`, `Context`, `Zustand` or `Redux Toolkit` according to complexity.
- [ ] **Single Source of Truth**: Verify that same data is not kept as state in multiple places (Derivable state).

### Phase 2: Implementation & Sync
- [ ] **Action/Mutation Design**: Define clear functions or reducers for state updates.
- [ ] **Effect Syncing**: Manage side effects of state changes (API calls, LocalStorage) in a controlled way.
- [ ] **Persistence Strategy**: Make necessary states (e.g. User session) persistent with `persist` middleware or custom logic.

### Phase 3: Optimization & Debugging
- [ ] **Selector Usage**: If using Zustand/Redux, select only necessary state parts (`selectors`) to prevent re-renders.
- [ ] **DevTools Audit**: Monitor state changes step by step using Redux DevTools or similar tools.
- [ ] **Testing**: Verify with unit tests whether state changes yield expected results.

### Checkpoints
| Phase | Verification                                              |
| ----- | --------------------------------------------------------- |
| 1     | Is State object mutated directly? (Immutability check)    |
| 2     | Does State hierarchy lead to "Prop Drilling" problem?     |
| 3     | Are Loading and Error states reflected to user correctly? |
