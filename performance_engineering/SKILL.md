...
> **Source:** [k6 Methodology](https://k6.io/docs/test-types/introduction/) & [The Art of Capacity Planning](https://www.oreilly.com/library/view/the-art-of/9781491939207/)

### Phase 1: Planning & SLOs
- [ ] **Goal**: What is the purpose of the test? (Smoke, Load, Stress, Soak?).
- [ ] **SLOs**: Define success criteria (e.g., p95 latency < 200ms, Error rate < 1%).
- [ ] **Environment**: How similar is the test environment to Prod? (Determine scaling factor).

### Phase 2: Scripting & Execution
- [ ] **User Journey**: Simulate real user behavior (Login -> Browse -> Buy).
- [ ] **Data Driven**: Feed the test with dynamic data from CSV, not static data (To bypass cache).
- [ ] **Ramp-up**: Increase traffic gradually, not suddenly (For system warm-up).

### Phase 3: Analysis & Optimization
- [ ] **Correlation**: What was the status of CPU/Memory/DB metrics at the time of error?
- [ ] **Bottleneck**: Where is the bottleneck? (App Code, DB, Network, or Load Injector itself?).
- [ ] **Report**: Prepare a report containing technical and executive summaries.

### Checkpoints
| Phase | Verification                                                                      |
| ----- | --------------------------------------------------------------------------------- |
| 1     | Is test data (Database seed) of sufficient volume?                                |
| 2     | Did Load Generator (Test machine) enter CPU bottleneck? (Risk of False negative). |
| 3     | Are 3rd party APIs (Stripe, Twilio) mocked? (Cost and ban risk).                  |
