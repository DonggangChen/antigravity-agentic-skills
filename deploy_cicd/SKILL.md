...
| Feature Flags | Code-based toggle                |
...
---
name: deploy_cicd
description: Vercel deployment, Edge Functions, Next.js optimization and ISR configuration guide.
metadata:
  skillport:
    category: deployment
    tags:
...
> Vercel deployment and Next.js optimization guide.
...
---
name: deploy_cicd
description: Railway container deployment, Docker, multi-service and auto-scaling configuration guide.
metadata:
  skillport:
    category: deployment
    tags:
...
> Railway container deployment guide.
...
Create a separate Railway service for each service.

---

## 🔄 Workflow

> **Source:** [DORA Research (DORA.dev)](https://dora.dev/) & [GitHub Actions Hardening Guide](https://docs.github.com/en/actions/security-guides/security-hardening-for-github-actions)

### Phase 1: Pipeline Orchestration & Security
- [ ] **Workflow Design**: Separate Test, Security (SAST), and Deploy steps into logical jobs.
- [ ] **Security Hardening**: Apply `permissions: read-all` or principle of least privilege. Pin 3rd party actions with commit SHA.
- [ ] **Secrets Management**: NEver store sensitive data in YAML; use GitHub Secrets or HashiCorp Vault.

### Phase 2: Testing & Artifact Management
- [ ] **Parallel Testing**: Run tests in parallel across different environments and versions using matrix builds.
- [ ] **Caching**: Improve pipeline time by 40%+ by caching `npm` or `pip` dependencies.
- [ ] **Artifact Creation**: Create a deployable package (Docker image/Binary) and upload to a registry (ECR/GCR).

### Phase 3: Deployment & DORA Tracking
- [ ] **Deployment Strategy**: Select Canary or Blue-Green strategy. Minimize downtime with rolling updates.
- [ ] **Environments**: Add `environment protection rules` (manual approval) before Production deploy.
- [ ] **Metric Collection**: Automatically track DORA metrics (Deployment Frequency, Lead Time for Changes).

### Checkpoints
| Phase | Verification                                                    |
| ----- | --------------------------------------------------------------- |
| 1     | Does the system remain in a safe state when the pipeline fails? |
| 2     | Do automated "Linter" and "Unit Test" runs on every PR?         |
| 3     | Is there an automated "Smoke Test" after deploy?                |
