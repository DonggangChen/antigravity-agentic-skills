...
> **Source:** [LangChain RAG Docs](https://python.langchain.com/docs/use_cases/question_answering/) & [Pinecone RAG Guide](https://www.pinecone.io/learn/retrieval-augmented-generation/)

### Phase 1: Ingestion Pipeline
- [ ] **Chunking**: Split using RecursiveCharacterTextSplitter maintaining semantic integrity (Overlap %10-20).
- [ ] **Embedding**: Choose domain-appropriate model (OpenAI `text-embedding-3` or local `bge-m3`).
- [ ] **Metadata**: Add date, source, author info to vector (For filtering).

### Phase 2: Retrieval Strategy
- [ ] **Hybrid Search**: Combine Keyword (BM25) + Vector (Cosine) search (Reciprocal Rank Fusion).
- [ ] **Re-ranking**: Re-rank top 50 results with Cross-Encoder (Cohere/BGE-Reranker).
- [ ] **Query Transformation**: Enrich user question ("this", "that") based on conversation history (HyDE).

### Phase 3: Generation & Evaluation
- [ ] **Prompting**: Provide context with "Answer only based on this information" directive.
- [ ] **Eval**: Measure "Context Relevance" and "Answer Faithfulness" with RAGAS or TruLens.

### Checkpoints
| Phase | Verification                                                         |
| ----- | -------------------------------------------------------------------- |
| 1     | Does vector database sync when document is updated?                  |
| 2     | Can it say "I don't know" to irrelevant questions?                   |
| 3     | Which line in the source document is the answer based on (Citation)? |
