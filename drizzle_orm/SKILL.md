...
> **Source:** [Drizzle Kit Migrations](https://orm.drizzle.team/docs/kit-overview)

### Phase 1: Schema Definition
- [ ] **Types**: Select correct data types like `text`, `integer` (SQLite/D1 compatible).
- [ ] **Constraints**: Define `notNull()`, `unique()`, `primaryKey()` constraints.
- [ ] **Relations**: Specify relations between tables using `relations()` function.

### Phase 2: Migration Lifecycle
- [ ] **Generate**: Generate SQL with `drizzle-kit generate`.
- [ ] **Review**: Manually check generated `.sql` file (Risk of data loss?).
- [ ] **Apply**: Test locally first with `wrangler d1 migrations apply --local`.

### Phase 3: Query Implementation
- [ ] **Queries**: Write type-safe queries with `.select().from()`.
- [ ] **Performance**: Prevent N+1 problem using `include` or `with`.
- [ ] **Batch**: Wrap batch operations in `db.batch([])`.

### Checkpoints
| Phase | Verification                                                            |
| ----- | ----------------------------------------------------------------------- |
| 1     | Is the `migrations` folder under version control (Git)?                 |
| 2     | Was it tested with `--local` before pushing to production (`--remote`)? |
| 3     | Are sensitive data read via `.env`?                                     |
