...
> **Source:** [Google Web Fundamentals](https://developers.google.com/web/fundamentals/design-and-ux/responsive) & [MDN Responsive Design](https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Responsive_Design)

### Phase 1: Modern Layout Techniques
- [ ] **Container Queries**: Write styles based on width of the area component resides in (`@container`), not screen width (`@media`).
- [ ] **CSS Grid & Subgrid**: Use Grid for complex 2D layouts. Use `subgrid` for nested alignment.
- [ ] **Fluid Typography**: Use fluid fonts with `clamp(1rem, 5vw, 2rem)` instead of fixed px/rem.

### Phase 2: Interaction & Adaptation
- [ ] **Interaction Queries**: Change style not just by screen size but by input type (`@media (hover: hover)`) (Touch device optimization).
- [ ] **Picture Element**: Serve different images (`<picture>`, `srcset`) for different screens (Art Direction & Performance).
- [ ] **Dark Mode**: Support system theme with `prefers-color-scheme`.

### Phase 3: Verification
- [ ] **Device Lab**: Test on real iOS and Android devices, not just Chrome DevTools (for 100vh issue etc.).
- [ ] **Zoom**: Verify layout doesn't break and remains readable when page is zoomed 200%.
- [ ] **Landscape**: Check if layout continues to work when mobile device is turned sideways.

### Checkpoints
| Phase | Verification                                                                   |
| ----- | ------------------------------------------------------------------------------ |
| 1     | Is there Horizontal Scroll? (Is there content overflowing?).                   |
| 2     | Are Clickable areas (Touch Targets) at least 44x44px?                          |
| 3     | Is CLS (Cumulative Layout Shift) under 0.1? (Is there jumping while loading?). |
