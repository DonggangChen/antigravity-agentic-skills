...
- [ ] **Data Sensitivity**: If high security required -> Short-lived tokens + Rotation.
- [ ] **Scale**: If horizontal scaling required -> Stateless JWT.

### Phase 2: Core Implementation
- [ ] **Storage**: Database schema (Users, RefreshTokens).
- [ ] **Hashing**: `bcrypt` or `argon2` setup.
- [ ] **Middleware**: `verifyToken` or `requireAuth` functions.

### Phase 3: Hardening (Hard-Requirements)
- [ ] **Transport**: HTTPS mandatory.
- [ ] **Rate Limiting**: Login endpoint protection (Brute-force).
- [ ] **Input Validation**: Email/Password regex (Zod/Joi).

### Phase 4: Audit Steps
- [ ] **Token Storage**: Is access token in LocalStorage? (If yes, XSS risk -> Use HttpOnly Cookie).
- [ ] **CSRF**: Is there protection on state modifier requests?
- [ ] **Logs**: Are passwords appearing in logs? (Must never happen).

### Checkpoints
| Phase | Verification                                          |
| ----- | ----------------------------------------------------- |
| 1     | Strategy fits the architecture (Not Over-engineering) |
| 2     | Passwords are stored hashed                           |
| 3     | Rate Limit active on Login endpoint                   |
| 4     | Penetration test steps (XSS/CSRF) clean               |
