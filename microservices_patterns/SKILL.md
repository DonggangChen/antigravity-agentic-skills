...
> **Source:** [Microservices.io Patterns](https://microservices.io/patterns/index.html)

### Phase 1: Pattern Selection
- [ ] **Data Consistency**: Select **2PC** (rarely) if strong consistency is needed, **Saga** for eventual consistency.
- [ ] **Querying**: Apply **CQRS** or **API Graph Composition** if complex joins are needed.
- [ ] **Resilience**: Define **Circuit Breaker** and **Bulkhead** for external dependencies.

### Phase 2: Implementation
- [ ] **Idempotency**: Establish idempotency key mechanism for all "Retryable" operations.
- [ ] **Outbox Pattern**: Database write and event publish must be atomic (Transaction Log Tailing or Polling).
- [ ] **Sidecar**: Delegate cross-cutting concerns (logging, auth, tracing) to sidecar proxy.

### Phase 3: Validation
- [ ] **Chaos Testing**: Test if resilience patterns are working by turning off dependencies.
- [ ] **Contract Testing**: Verify service contracts with tools like Pact.

### Checkpoints
| Phase | Verification                                                          |
| ----- | --------------------------------------------------------------------- |
| 1     | Are "Compensating Transactions" (rollback) defined in Saga pattern?   |
| 2     | Does fallback mechanism work when Circuit Breaker opens (Open State)? |
| 3     | Is event schema evolution (Schema Registry) backward compatible?      |
