...
> **Source:** [SpringDoc Official Documentation](https://springdoc.org/)

### Phase 1: Configuration
- [ ] **Dependency**: Add `springdoc-openapi-starter-webmvc-ui` (v2.x for Spring Boot 3).
- [ ] **Properties**: Fix `springdoc.api-docs.path` and `swagger-ui.path` values (if using custom path).
- [ ] **Platform**: Pay attention to WebMVC vs WebFlux distinction (dependency is different).

### Phase 2: Documentation Layer
- [ ] **Controller**: Group with `@Tag`, explain each endpoint with `@Operation`.
- [ ] **Models**: Define DTOs with `@Schema`, add validation annotations (`@NotNull`) (reflects automatically).
- [ ] **Security**: Define global security scheme (JWT/OAuth2) and add `@SecurityRequirement` to endpoints.

### Phase 3: Enhancement
- [ ] **Examples**: Use `@ExampleObject` to provide realistic examples for request/response bodies.
- [ ] **Error Handling**: Document error response formats in Global Exception Handler with `@ApiResponse`.
- [ ] **Generation**: Generate `openapi.json` with `springdoc-openapi-maven-plugin` in CI/CD pipeline.

### Checkpoints
| Phase | Verification                                                    |
| ----- | --------------------------------------------------------------- |
| 1     | Does `/v3/api-docs` return JSON?                                |
| 2     | Does "Try it out" button work in Swagger UI (CORS/Auth issues)? |
| 3     | Do Enum values and required fields appear correctly in docs?    |