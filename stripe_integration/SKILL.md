...
> **Source:** [Stripe Integration Builder](https://stripe.com/docs/checkout/quickstart) & [Stripe Webhook Best Practices](https://stripe.com/docs/webhooks/best-practices)

### Phase 1: Product & Checkout Setup
- [ ] **Product Definition**: Define Product and Price objects via Dashboard or API.
- [ ] **Checkout Integration**: Set up appropriate choice between `Stripe Checkout` (Hosted) or `Payment Element` (Custom).
- [ ] **SCA Compliance**: Add required SCA (Strong Customer Authentication) steps for European (3D Secure) payments.

### Phase 2: Webhook & Background Processing
- [ ] **Webhook Listener**: Set up a secure webhook endpoint for payment success (`payment_intent.succeeded`) or subscription status changes.
- [ ] **Signature Verification**: Verify that incoming requests are from Stripe using `endpoint secret`.
- [ ] **Idempotency**: Use `Stripe-Idempotency-Key` or database check to prevent processing same webhook request multiple times.

### Phase 3: Exception Handling & Fulfillment
- [ ] **Payment Failures**: Notify user of payment failures and manage the process (e.g. preserve Cart).
- [ ] **Order Fulfillment**: Complete order when payment is confirmed, update database and send email.
- [ ] **Customer Portal**: Integrate "Customer Portal" link where users can manage their subscriptions.

### Checkpoints
| Phase | Verification                                                               |
| ----- | -------------------------------------------------------------------------- |
| 1     | Are payment amounts sent in "Cents" (if USD 100 = $1.00)?                  |
| 2     | Is Webhook endpoint running over HTTPS in production?                      |
| 3     | Is entire flow (Success/Fail/3DS) tested with "4242..." card in Test mode? |
