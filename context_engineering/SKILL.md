...
> **Source:** [Anthropic Context Optimization Guide](https://docs.anthropic.com/en/docs/build-with-claude/context-caching)

### Phase 1: Analysis & Measurement
- [ ] **Audit**: Tokenize current prompt and conversation history.
- [ ] **Identify**: Identify static blocks (System prompt, tool definitions).
- [ ] **Budget**: Mark parts exceeding 70% of total limit.

### Phase 2: Optimization Implementation
- [ ] **Cache**: Move static blocks (System Prompt + Tools) to the top (KV-Cache Friendly).
- [ ] **Compact**: Create old message summaries (Compaction).
- [ ] **Mask**: Reference large tool outputs (Observation Masking) with `id`.

### Phase 3: Performance Validation
- [ ] **Recall Test**: Check for lost information with "Needle in a haystack" test.
- [ ] **Latency**: Measure improvement in TTFT (Time to First Token).

### Checkpoints
| Phase | Verification                                                               |
| ----- | -------------------------------------------------------------------------- |
| 1     | Does static content remain unchanged in every request? (No Timestamp etc.) |
| 2     | Are tool outputs summarized instead of raw JSON?                           |
| 3     | Is Cache Hit Rate > 80%?                                                   |
