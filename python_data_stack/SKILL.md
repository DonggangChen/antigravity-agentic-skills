...
> **Source:** [Modern Data Stack](https://moderndatastack.xyz/) & [Polars Documentation](https://pola.rs/)

### Phase 1: Local Data Processing (Modern OLAP)
- [ ] **Polars**: Use Rust-based `Polars` for big data (GB+) instead of Pandas (for Lazy Evaluation and Parallelism).
- [ ] **DuckDB**: Use `DuckDB` for local SQL analytics and querying Parquet files (In-process OLAP).
- [ ] **Format**: Store data in `Parquet` or `Arrow` format instead of CSV (Speed and compression).

### Phase 2: Analysis & Visualization
- [ ] **Notebooks**: Perform interactive analysis with Jupyter Notebook (or VS Code Notebooks), but production code should be in `.py` files.
- [ ] **Viz**: Use `Altair` or `Plotnine` (Grammar of Graphics) for declarative visualization instead of Matplotlib.
- [ ] **Profiling**: Generate `ydata-profiling` reports to understand data quality.

### Phase 3: Production Pipeline
- [ ] **Orchestration**: Use `Prefect` or `Dagster` for ETL jobs (Modern and lightweight compared to Airflow).
- [ ] **Validation**: Use `Pydantic` or `Great Expectations` to validate data schemas and quality.
- [ ] **Environment**: Use `uv` or `conda` environment to isolate dependencies.

### Checkpoints
| Phase | Verification                                                                        |
| ----- | ----------------------------------------------------------------------------------- |
| 1     | Does data processing fit in memory (RAM)? (If not, use Polars Streaming or DuckDB). |
| 2     | Are repeated analyses converted to modular functions?                               |
| 3     | Is data privacy (PII) checked?                                                      |
