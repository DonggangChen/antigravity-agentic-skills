...
> **Source:** [React 19 Documentation](https://react.dev/blog/2024/12/05/react-19) & [TanStack Query Best Practices](https://tanstack.com/query/latest/docs/framework/react/overview)

### Phase 1: Architecture & Server/Client Split
- [ ] **Component Classification**: Determine Server (RSC) or Client (RCC) roles of components. Keep data fetching at RSC level if possible.
- [ ] **State Hierarchy**: Decide where state should reside (Local, Context, Global - Zustand/Redux).
- [ ] **Strict Type Definition**: Define detailed TypeScript interface/types for all props and states.

### Phase 2: Implementation & Interaction
- [ ] **Modern Data Fetching**: Integrate data loading processes with Suspense using `use()` hook or TanStack Query.
- [ ] **Form Actions**: Setup form management and optimistic updates using React 19 `useActionState` and `useFormStatus`.
- [ ] **Memoization Audit**: Apply `useMemo` and `useCallback` only where expensive calculations and stable references are required.

### Phase 3: Performance & A11y Verification
- [ ] **Profiling Check**: Detect and fix unnecessary re-renders with React DevTools Profiler.
- [ ] **Accessibility Audit**: Verify ARIA roles, focus management and screen reader compatibility (WCAG).
- [ ] **Error Boundaries**: Do not leave user with a white screen by adding `ErrorBoundary` to critical points of the application.

### Checkpoints
| Phase | Verification                                                     |
| ----- | ---------------------------------------------------------------- |
| 1     | Are `key` props stable and unique IDs instead of index?          |
| 2     | Is there any hydration error during Server Side Rendering (SSR)? |
| 3     | Is there a cleanup function in Side Effects (`useEffect`)?       |
