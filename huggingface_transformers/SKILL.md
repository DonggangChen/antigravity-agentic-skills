...
> **Source:** [Hugging Face Course](https://huggingface.co/course) & [Production Guide](https://huggingface.co/docs/transformers/performance)

### Phase 1: Model Selection
- [ ] **Task**: Choose the most suitable architecture for your task (Encoder: classification, Decoder: generation).
- [ ] **License**: Does the model have permission for commercial use (Apache 2.0 vs Llama Community)?
- [ ] **Size**: Balance parameter count vs performance (7B is usually sufficient).

### Phase 2: Optimization pipeline
- [ ] **Quantization**: Use 4-bit / 8-bit quantization (BitsAndBytes) for inference.
- [ ] **Batching**: Process in batches, not one by one (GPU efficiency).
- [ ] **Format**: Convert to ONNX or TensorRT format for production.

### Phase 3: Deployment
- [ ] **Cache**: Do not bake model weights and tokenizer into docker image, use volume.
- [ ] **Token Limits**: Determine strategy for inputs exceeding context window limit (chunking).

### Checkpoints
| Phase | Verification                                     |
| ----- | ------------------------------------------------ |
| 1     | Does the model fit into GPU memory (OOM error)?  |
| 2     | Is inference time (Latency) under the target?    |
| 3     | Is Tokenizer compatible with Model (same vocab)? |
