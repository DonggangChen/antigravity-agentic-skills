...
> **Source:** [Cookiecutter Documentation](https://cookiecutter.readthedocs.io/) & [UV Package Manager](https://github.com/astral-sh/uv)

### Phase 1: Stack & Tool Selection
- [ ] **Python**: Use `uv` as the 2025 standard (instead of pip/poetry). Keep `pyproject.toml` as the single configuration file.
- [ ] **Node.js**: Prefer `pnpm` (disk efficiency) or `bun` (speed).
- [ ] **Monorepo**: Install `Turborepo` or `Nx` if there are multiple packages.

### Phase 2: Scaffolding (Opinionated Defaults)
- [ ] **Linter/Formatter**: `Ruff` for Python (single tool, all-in-one), `Biome` or `ESLint` + `Prettier` for JS.
- [ ] **Git Hooks**: Enforce pre-commit quality control with `pre-commit` (Python) or `husky` (JS).
- [ ] **Editor config**: Ensure team works in same environment with `.editorconfig` and `.vscode/extensions.json`.

### Phase 3: CI/CD & Documentation
- [ ] **GitHub Actions**: Add `ci.yml` (Test & Lint) and `release.yml` (NPM/PyPI publish) workflows from the start.
- [ ] **README**: Add "How to Install", "How to Test", "How to Contribute" sections as template.
- [ ] **License**: Create suitable license file (MIT, Apache 2.0) for the project type.

### Checkpoints
| Phase | Verification                                                                     |
| ----- | -------------------------------------------------------------------------------- |
| 1     | Does project run immediately after `git clone` and `npm install` (or `uv sync`)? |
| 2     | Is CI pipeline green on first commit?                                            |
| 3     | Are secret keys (`.env`) added to gitignore?                                     |
