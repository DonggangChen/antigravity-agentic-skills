...
> **Source:** [HuggingFace TRL Docs](https://huggingface.co/docs/trl/index) & [DeepLearning.ai LLM Engineering](https://www.deeplearning.ai/)

### Phase 1: Preparation (Data-Centric AI)
- [ ] **Data Cleaning**: Deduplicate the dataset and perform quality control (PII cleaning).
- [ ] **Format**: Convert dataset to model-compatible format (ShareGPT, Alpaca etc.).
- [ ] **Baseline**: Measure and record base model performance (Zero-shot).

### Phase 2: Training (Parameter Efficient)
- [ ] **LoRA/QLoRA**: Use LoRA (Rank 16-64) instead of full fine-tuning (Less VRAM, %95+ performance).
- [ ] **Monitoring**: Monitor Loss curves and eval metrics live with WandB or MLflow.
- [ ] **Checkpointing**: Save model weights every epoch or specific step.

### Phase 3: Alignment & Evaluation
- [ ] **Alignment**: Align with human preferences using DPO or PPO after SFT if needed.
- [ ] **Evaluation**: Perform automatic and manual tests using `llm_evaluation` skill.
- [ ] **Merging**: Merge LoRA adapters to base model and quantize (GGUF/AWQ).

### Checkpoints
| Phase | Verification                                                                 |
| ----- | ---------------------------------------------------------------------------- |
| 1     | Is Training Loss decreasing while Validation Loss increases (Overfitting)?   |
| 2     | Does the model experience "Catastrophic Forgetting" (Lost old capabilities)? |
| 3     | Are Inference speed and memory (VRAM) suitable for deployment environment?   |
